(() => {
  ...
  // ---------- État ----------
  const etat = { cat: TOUTES, filtres: new Set(), q: "" };
  ...
  function afficherFiltres() {
    ...
    $("#filtres").innerHTML = puces.map((p) =>
      `<button type="button" class="puce" data-filtre="${esc(p.id)}"
  ...
  function ouvrirFiche(id, majUrl = true) {
    ...
    if (!r) return;
    let portions = r.portions || 1;

    const ingredientsHTML = () => (r.ingredients || []).map((ing, i) => {
      const q = typeof ing.qte === "number" ? ing.qte * portions / (r.portions || 1) : ing.qte;
      const texte = [formatQte(q), ing.unite, ing.nom].filter((x) => x !== "" && x != null).join(" ");
      return `<li><label><input type="checkbox" data-ing="${i}"><span>${esc(texte)}</span></label></li>`;
    }).join("");
    ...
    const majPortions = () => {
      $("#nb-portions").textContent = `${portions} portion${portions > 1 ? "s" : ""}`;
      $("#liste-ing").innerHTML = ingredientsHTML();
    };
    majPortions();
  ...
  function roulette() {
    ...
    const candidates = filtrer().length ? filtrer() : RECETTES;
